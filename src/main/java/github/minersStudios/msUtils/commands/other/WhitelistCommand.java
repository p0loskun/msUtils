		...
		if (args.length > 1 && args[1].length() > 2) {
			...
			if (args[0].equalsIgnoreCase("add")) {
				if (PlayerUtils.addPlayerToWhitelist(offlinePlayer, args[1]))
					return ChatUtils.sendFine(sender, "Игрок : \"" + playerInfo.getGrayIDGreenName() + " (" + args[1] + ")\" был добавлен в белый список");
				return ChatUtils.sendWarning(sender, "Игрок : \"" + playerInfo.getGrayIDGoldName() + " (" + args[1] + ")\" уже состоит в белом списке");
			}
			if (args[0].equalsIgnoreCase("remove")) {
				if (PlayerUtils.removePlayerFromWhitelist(offlinePlayer, args[1]))
					return ChatUtils.sendFine(sender, "Игрок : \"" + playerInfo.getGrayIDGreenName() + " (" + args[1] + ")\" был удалён из белого списка");
				return ChatUtils.sendWarning(sender, "Игрок : \"" + playerInfo.getGrayIDGoldName() + " (" + args[1] + ")\" не состоит в белом списке");
			}
			return false;
		}

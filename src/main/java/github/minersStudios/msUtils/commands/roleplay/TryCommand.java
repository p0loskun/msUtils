            ...
            if (args.length < 1) {
                ...
                if (!playerInfo.isMuted()) {
                    String message = CommandUtils.extractMessage(args, 0),
                            result = CommandUtils.randomResult();
                    CommandUtils.sendMessage(player, 25, " ꀓ " + ChatColor.GOLD + "*" + ChatColor.GRAY + " [" + playerInfo.getID() + "] " + ChatColor.GOLD + playerInfo.getFirstname() + " " + playerInfo.getLastname() + " " + message + " " + result + ChatColor.GOLD + "*");
